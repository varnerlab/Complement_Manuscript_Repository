...
Thus targeting C3 and C5 directly will require high dosage of drugs. It is also well known that eculizumab dosage needs to be adjusted while treating for Atypical Hemolytic-Uremic Syndrome (aHUS), a disease that is caused due to uncontrolled complement activation \cite{noris2014dynamics}. The issue of high dosage can potentially be circumvented by targeting convertases or fragile mechanisms that involve C3, C5 or their activated components.
Our analysis shows that formation and assembly of these convertases are sensitive mechanisms that strongly impact downstream proteins like C5a. Formation of convertases is inhibited by targeting upstream protease complexes like MASP-1,2 from lectin pathway (or C1r, C1s from classical pathway).
...
The model equations were implemented in Julia and solved using the \texttt{SUNDIALS} routine [REFs].
...
We estimated a single initial parameter set using the using Dynamic Optimization with Particle Swarms (DOPS) technique [REF].
DOPS minimized the squared residual between simulated and experimental C3a and C5a measurements with and without zymosan as a single objective formulation.
This initial best fit set then served as the starting point for multiobjective ensemble generation using Pareto Optimal Ensemble Technique in the Julia programming language (JuPOETs) [REF].
JuPOETs is a multiobjective approach which integrates simulated annealing with Pareto optimality to estimate model ensembles on or near the optimal tradeoff surface between competing training objectives. JuPOETs minimized training objective functions of the form:
...
\end{equation}
The symbol $\hat{\mathcal{M}}_{ij}$ denotes an experimental observation (from training set $j$) while the symbol $\hat{y}_{ij}$ denotes the simulation output (from training objective $j$). The quantity $i$ denotes the sampled time-index and $\mathcal{T}_{j}$ denotes the number of time points for experiment $j$.
In this study, we minimized two training objectives, the sum of the C3a and C5a residual w/o zymosan (objective )
